...
# 2024-12-27
ウィンドウをリソースの一種として扱おうとしたが、
- そもそもリソースの利点は所有権をGUIに預けられることだけ。
- 当初は「参照の生死を確認できる」のもメリットだと考えていたが、Rustのような綺麗なロック機構が作れない（結局最後は参照を取る）ためあまり意味がないことが分かった。
- ウィンドウの生死は十分に自分で管理できる。
という理由で廃止した。

言語仕様に助けられる分、Rcはshared_ptrよりもずっと安全であるのだということを実感できた。

# 2024-12-28
一つのブロックに２つ以上のボールが同時にぶつかると、ブロックを削除するコールバックが複数回呼ばれてしまうことを突き止めた。

# 2025-01-04
変数の寿命が原因で、テクスチャが表示されない問題に悩まされた。
今まで開発してきて、バグの原因は①こうした寿命の問題と②物体の二重削除の２パターンが多いと感じる。


# 2025-02-24
ロガーとsolの相性が悪かった。ライブラリ同士の衝突にも注意。

# 2025-02-28
mp3の音声データ容量が意外と大きかった（3分くらいの曲で7MB）。
ビットレートを64kbpsに落とすことで1.5MBくらいまで縮小できた。
